...
const TX_ICON: Record<string, { sym: string; label: string }> = {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-green-600 hover:bg-green-500 gap-2 font-semibold text-sm h-10">
...
function WithdrawDialog({ phone, max }: { phone?: string | null; max: number }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="border-green-700/50 text-green-300 hover:bg-green-900/25 gap-2 font-semibold text-sm h-10">
...
export default function DashboardPage() {
    ...
    return (
      <AppLayout>
        <div className="space-y-4 animate-pulse">
          <div className="h-36 rounded-2xl bg-[#0d1a10]" />
          <div className="grid grid-cols-2 gap-3">
            {[...Array(4)].map((_, i) => <div key={i} className="h-20 rounded-2xl bg-[#0d1a10]" />)}
          </div>
          <div className="h-10 rounded-2xl bg-[#0d1a10]" />
          <div className="h-48 rounded-2xl bg-[#0d1a10]" />
          <div className="h-48 rounded-2xl bg-[#0d1a10]" />
        </div>
      </AppLayout>
    );
  ...
  const stats = [
    { label: "Available", value: fmt(dash?.availableBalance ?? 0), icon: Wallet,    color: "text-green-400",  border: "border-green-800/40",  iconBg: "bg-green-900/30"  },
    { label: "Staked",    value: fmt(dash?.totalStaked ?? 0),      icon: TrendingUp, color: "text-sky-400",    border: "border-sky-900/40",    iconBg: "bg-sky-900/20"    },
    { label: "Earnings",  value: fmt(dash?.totalEarnings ?? 0),    icon: DollarSign, color: "text-yellow-400", border: "border-yellow-900/40", iconBg: "bg-yellow-900/20" },
    { label: "Referrals", value: fmt(dash?.referralRewards ?? 0),  icon: Gift,       color: "text-purple-400", border: "border-purple-900/40", iconBg: "bg-purple-900/20" },
  ];

  const quickActions = [
    { label: "Stake",          sub: "Earn returns",  href: "/staking",       icon: TrendingUp, color: "text-green-400" },
    { label: "Transactions",   sub: "View history",  href: "/transactions",  icon: BarChart3,  color: "text-sky-400"   },
    { label: "Referrals",      sub: "Earn 5%",       href: "/referrals",     icon: Users,      color: "text-purple-400"},
    ...
    <AppLayout>
      <div className="space-y-4">
        ...
        <div className="rounded-2xl bg-[#0d2010] border border-green-800/30 p-5">
          <div className="flex flex-col gap-4">
            ...
            <div>
              ...
              <h1 className="text-3xl font-black text-white">{fmt(totalPortfolio)}</h1>
              {roi > 0 && (
                <span className="inline-flex items-center gap-1 mt-2 px-2.5 py-1 rounded-full bg-[#0a2510] border border-green-700/40 text-green-400 text-xs font-bold">
                  ↑ {roi.toFixed(1)}% ROI · {activeStakes.length} active stake{activeStakes.length !== 1 ? "s" : ""}
                </span>
              )}
              {roi === 0 && activeStakes.length > 0 && (
        ...
        <div className="grid grid-cols-2 gap-3">
          {stats.map((s) => (
            <div key={s.label} className={`bg-[#0a1208] border ${s.border} rounded-2xl p-4`}>
              ...
              <p className={`text-sm font-black ${s.color} leading-snug`}>{s.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {quickActions.map((a) => (
            <Link key={a.href} href={a.href}>
              <div className="flex flex-col items-center text-center gap-2 py-3 px-1 rounded-2xl bg-[#0a1208] border border-green-900/20 hover:border-green-800/40 transition-colors cursor-pointer">
                <div className="w-9 h-9 rounded-xl bg-[#060d08] flex items-center justify-center">
                  <a.icon className={`w-4 h-4 ${a.color}`} />
                </div>
                <p className="text-[10px] font-semibold text-gray-300 leading-tight">{a.label}</p>
              </div>
            </Link>
          ))}
        </div>
          ...
          <div className="flex items-center justify-between px-4 py-3.5 border-b border-green-900/20">
            <div className="flex items-center gap-2">
              ...
              {activeStakes.length > 0 && (
                <span className="bg-green-900/50 text-green-400 text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                  {activeStakes.length}
                </span>
              )}
            </div>
            ...
            {activeStakes.length === 0 ? (
              ...
              <div className="space-y-3">
                    ...
                    <div key={stake.id} className="bg-[#060d08] border border-green-900/20 rounded-xl p-4">
                      <div className="flex items-start justify-between mb-3">
                        <div className="min-w-0 flex-1 pr-2">
                          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                            ...
                            <span className="text-[10px] bg-[#0a2510] text-green-400 border border-green-800/40 rounded-full px-1.5 py-0.5 font-semibold whitespace-nowrap">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-[10px] text-gray-600">
                          ...
                          <span className="text-green-400 font-semibold">{elapsed}d of {total}d</span>
                        </div>
                        <div className="h-1.5 bg-[#0a1208] rounded-full overflow-hidden">
                          <div
                ...
                {activeStakes.length > 5 && (
                  <Link href="/staking">
                    <p className="text-center text-xs text-green-400 hover:text-green-300 py-1">
          ...
          <div className="p-2">
            {(dash?.recentTransactions?.length ?? 0) === 0 ? (
              <div className="text-center py-10">
                <div className="w-10 h-10 rounded-2xl bg-[#060d08] flex items-center justify-center mx-auto mb-3">
              ...
              <div>
                {dash?.recentTransactions?.slice(0, 8).map((tx: any) => {
                  ...
                  const info   = TX_ICON[tx.type] ?? { sym: "·", label: tx.type };
                  return (
                    <div key={tx.id}
                      className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-[#060d08] transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-gray-600 mt-0.5">{info.label} · {timeAgo(tx.createdAt)}</p>
                      </div>
                      <div className="text-right shrink-0">
        ...
        {(dash?.totalEarnings ?? 0) > 0 && (
          <div className="rounded-2xl bg-[#0a1a0d] border border-green-700/25 p-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Total Staking Earnings</p>
                <p className="text-xl font-black text-green-400">{fmt(dash?.totalEarnings ?? 0)}</p>
                {roi > 0 && <p className="text-xs text-yellow-400 mt-0.5">+{roi.toFixed(2)}% return on investment</p>}
              </div>
              <Link href="/staking">
